<script setup lang="ts">
  ...
  import { MODE_PILL } from '@virtual-tcu/shared/utils/mode-colors'
...
<template>
  ...
  >
    ...
    <div class="flex items-center gap-5">
      <LocaleSwitcher />
      <div class="text-tcu-txt-muted flex items-center gap-2 text-sm">
        <span>{{ $t(connectionKey) }}</span>
        <span
          class="size-2 rounded-full transition-colors"
          :class="[
            connectionDotClass(connected, live) === 'live' &&
