...
SYS_DRIVE = os.getenv("SYSTEMROOT", "C:\\Windows")[:2]
dll_path = [
    f"{SYS_DRIVE}\\Program Files\\vJoy\\x64\\{DLL_FILENAME}",
    f"{SYS_DRIVE}\\Program Files (x86)\\vJoy\\x64\\{DLL_FILENAME}",
    f"{SYS_DRIVE}\\Program Files\\vJoy\\{DLL_FILENAME}",
    f"{SYS_DRIVE}\\Program Files (x86)\\vJoy\\{DLL_FILENAME}",
    f"C:\\Program Files\\vJoy\\x64\\{DLL_FILENAME}",
    os.path.dirname(__file__) + os.sep + DLL_FILENAME,
]
...
def AcquireVJD(rID):
    ...
    if result == 0:
        ...
        if status != VJD_STAT_FREE:
            msg = "Cannot acquire vJoy Device because it is not in VJD_STAT_FREE"
            raise vJoyFailedToAcquireException(msg)
